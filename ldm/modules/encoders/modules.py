...
class FrozenCLIPEmbedder(AbstractEncoder):
    ...
    def forward(self, text, return_pooler_output=False, return_tokens=False):
        batch_encoding = self.tokenizer(text, truncation=True, max_length=self.max_length, return_length=True,
                                        return_overflowing_tokens=False, padding="max_length", return_tensors="pt")
        tokens = batch_encoding["input_ids"].to(self.device)
        ...
        if return_pooler_output:
            ...
            tokens = self.tokenizer.convert_ids_to_tokens(batch_encoding.input_ids[0])
            return z, tokens
        else:
            return z

    def encode(self, text, return_pooler_output=False, return_tokens=False):
        return self(text, return_pooler_output, return_tokens)
