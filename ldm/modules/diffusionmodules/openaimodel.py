...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context, obj_hints, obj_masks, obj_vectors, wat_hints, wat_masks, wat_vectors):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, SpatialTransformer):
                x = layer(x, context, obj_hints, obj_masks, obj_vectors, wat_hints, wat_masks, wat_vectors)
            else:
                x = layer(x)
        return x
...
class UNetModel(nn.Module):
    def __init__(
        ...
        assert fuser_type in ["gatedSA","gatedSA2","gatedCA","objCA"]
    ...
    def forward(self, input):
        ...
        wat_hints, obj_hints = self.position_net( **grounding_input )  
        ...
        obj_masks_ori = grounding_input["obj_cond"]["obj_masks"]
        obj_vectors = grounding_input["obj_cond"]["obj_vectors"]
        wat_masks_ori = grounding_input["wat_cond"]["wat_masks"]
        wat_vectors = grounding_input["wat_cond"]["wat_vectors"]
        
        first_layer = True
        # Start forwarding
        hs = []
        for module in self.input_blocks:
            obj_masks = self._upsample(obj_masks_ori, h)
            wat_masks = self._upsample(wat_masks_ori, h)
            h = module(h, emb, context, obj_hints, obj_masks, obj_vectors, wat_hints, wat_masks, wat_vectors)
            if first_layer:
                h = h # + water_hint
                first_layer = False
            hs.append(h)
        ...
        obj_masks = self._upsample(obj_masks_ori, h)
        wat_masks = self._upsample(wat_masks_ori, h)
        h = self.middle_block(h, emb, context, obj_hints, obj_masks, obj_vectors, wat_hints, wat_masks, wat_vectors)
